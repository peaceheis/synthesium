...
class MatableGroup(Matable):
    ...
    def __init__(self, *matables: Union[tuple, list], **kwargs): 
        ...
        self.points = [matable.points() for matable in matables] #recursively adds points
        self.configure(kwargs)

    def add_matable(self, matable): 
        self.matables += matable
        self.points += matable.points()

    def get_matables_by_type(self, type: Matable.__class__): 
        ...
        """
        matables = []
        for matable in self.matables: #probably could be more compact as a list comprehension, but it's more readable this way.
    ...
    def points(self) -> "tuple": 
        return tuple([matable.points() for matable in self.matables])

    @points.setter
    def set_points(self, *points): #despite automatically raising an error on trying to run this, it's best to say WHY the error is being raised,
        #instead of python saying "invalid number of arguments" down the line
    ...
    @property
    def matables(self) -> "list[Matable]": 
