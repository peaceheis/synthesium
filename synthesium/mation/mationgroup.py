...
class MationGroup(Mation):
    ...
    def tick(self):
        ...
        return MatableGroup(*[mation.tick() for mation in self.mations if mation.is_active_at_frame(
            self.current_frame)])  # returns a MatableGroup of returned mations for each ticked mation
    ...
    def remove(self, *mations):
        ...
        self.mations = [mation for mation in self.mations if
                        mation not in mations]  # regenerate mationlist without the mations in the list.
        self.update_bounds()
